 ...
 * 順序：Powerhouse → Power Element
 * 注意：Elementor 在測試環境中可能不存在，Power Element 已處理此情況
 */
function _power_element_manually_load_plugin(): void {
	// 1. 載入 Powerhouse（提供 J7\WpUtils 工具庫）
	...
	// 2. 載入 Power Element plugin
	// Elementor 未載入時，ElementorWidgets\Register 的 hooks 不會被觸發（add_action 保護）
	// Shortcodes\Register 與 Bootstrap 仍可正常運作
	require dirname( __DIR__ ) . '/plugin.php';
}

tests_add_filter( 'muplugins_loaded', '_power_element_manually_load_plugin' );

/**
 * 在 plugins_loaded 後強制初始化 Bootstrap
 * 原因：PluginTrait::check_required_plugins() 會呼叫 is_j7rp_complete()
 * 但測試環境中 Powerhouse 是透過 muplugins_loaded 手動載入
 * 並未在 active_plugins DB 選項中登記，導致 is_j7rp_complete() 回傳 false
 * 進而阻止 Bootstrap::instance() 被呼叫
 * 解決：在 plugins_loaded 之後直接呼叫 Bootstrap::instance()
 */
function _power_element_force_bootstrap(): void {
	if ( class_exists( 'J7\PowerElement\Bootstrap' ) ) {
		\J7\PowerElement\Bootstrap::instance();
	}
}

tests_add_filter( 'plugins_loaded', '_power_element_force_bootstrap', 20 );

// 啟動 WP 測試套件
require "{$_tests_dir}/includes/bootstrap.php";

// 預先載入自訂 TestCase 基礎類別（確保在 WP_UnitTestCase 存在後才載入）
